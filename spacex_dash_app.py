...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        failures = len(filtered_df)-filtered_df['class'].sum()
        diction = {'values' : [success, failures], 'variable' : ['0', '1']}
        df_plot = pd.DataFrame(diction)
        fig = px.pie(df_plot, values = 'values', names = 'variable')
        #fig = px.pie(filtered_df, values = 'class', names = 'class', title = 'Total Success Launches for site' + str(entered_site))
        return fig
# TASK 4:
...
def get_scatter_plot(entered_site, entered_payload):
    if entered_site == 'ALL':
        ...
        mask2 = (spacex_df['Payload Mass (kg)'] < entered_payload[1]) & (spacex_df['Payload Mass (kg)'] > entered_payload[0])
